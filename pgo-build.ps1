...
param(
    ...
    [ValidateSet("ON", "OFF")]
    [string]$SibCiRun = $(if ($env:CI) { "ON" } else { "OFF" })
)

Write-Host "SIB_CI_RUN: $SibCiRun"
...
cmd /c "`"$vcvars`" && cmake -S . -B ./Build-Gen -DCMAKE_BUILD_TYPE=Release -DPGO_MODE=GENERATE -DPGO_DATA_DIR=`"$PgoDataDir`" -DSIB_CI_RUN=$SibCiRun"
...
if ($trainingBin) {
    cmd /c "`"$vcvars`" && `"$($trainingBin.FullName)`""
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} else {
...
cmd /c "`"$vcvars`" && cmake -S . -B ./Build -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=`"$InstallPath`" -DPGO_MODE=USE -DPGO_DATA_DIR=`"$PgoDataDir`" -DSIB_CI_RUN=$SibCiRun"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
