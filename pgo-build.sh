...
if [[ -n "${CI:-}" ]]; then
  SIB_CI_RUN="${SIB_CI_RUN:-ON}"
else
  SIB_CI_RUN="${SIB_CI_RUN:-OFF}"
fi
echo "SIB_CI_RUN: $SIB_CI_RUN"
...
if [[ "$COMPILER_KIND" == "gcc" ]]; then
  export GCOV_PREFIX="$(realpath ./Build-Gen)"
...
if [[ -z "$FINAL_BIN" ]]; then
  echo "ERROR: StringInt-Benchmarks binary not found under ./Build" >&2
  exit 1
fi
chmod +x "$FINAL_BIN"
echo "Final PGO-optimized binary: $FINAL_BIN"
echo "=== Running final PGO-optimized binary ==="
