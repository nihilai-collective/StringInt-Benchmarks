...
namespace vn_unchecked {
	...
	template<vn::detail::integer_types v_type> VN_INLINE static void copy_3_digits(char* __restrict buf, const v_type value) noexcept {
		uint32_t packed;
...
#if VN_ARCH_X64
		VN_INLINE static char* impl(char* __restrict buf VN_LIFETIME_BOUND, const v_type value) noexcept {
			return value < 100U
				? value < 10U ? (buf[0] = char(value) + static_cast<char>('0'), buf + 1) : (std::memcpy(buf, vn::detail::char_table_2_digit_data + value, 2ULL), buf + 2)
				: value < 100000000ULL		   ? value < 10000ULL
	...
	template<vn::detail::uint32_types v_type> struct to_chars_impl<v_type> {
		...
		VN_INLINE static char* impl(char* __restrict buf VN_LIFETIME_BOUND, const v_type value) noexcept {
			return value < 100U
				? value < 10U ? (buf[0] = char(value) + static_cast<char>('0'), buf + 1) : (std::memcpy(buf, vn::detail::char_table_2_digit_data + value, 2ULL), buf + 2)
				: value < 1000000U	  ? value < 10000U
					? value < 1000U ? (copy_3_digits(buf, value), buf + 3) : (std::memcpy(buf, vn::detail::char_table_4_digit_data + value, 4ULL), buf + 4)
					: value < 100000U ? to_chars_internal<v_type, 5ULL>::impl(buf, value)
									  : to_chars_internal<v_type, 6ULL>::impl(buf, value)
				...
				: value < 1000000000U ? to_chars_internal<v_type, 9ULL>::impl(buf, value)
...
#if VN_ARCH_X64
		VN_INLINE static char* impl(char* __restrict buf VN_LIFETIME_BOUND, const v_type value) noexcept {
			return value < 10U	 ? (buf[0] = char(value) + static_cast<char>('0'), buf + 1)
				: value < 1000U	 ? value < 100U ? (std::memcpy(buf, vn::detail::char_table_2_digit_data + value, 2ULL), buf + 2) : (copy_3_digits(buf, value), buf + 3)
				: value < 10000U ? (std::memcpy(buf, vn::detail::char_table_4_digit_data + value, 4ULL), buf + 4)
								 : to_chars_internal<v_type, 5ULL>::impl(buf, value);
		}
#else
	...
	struct to_chars_impl<v_type> {
		VN_INLINE static char* impl_negative(char* __restrict buf VN_LIFETIME_BOUND, const v_type value) noexcept {
...
namespace benchmarks {
	...
	template<uint64_t total_size, typename int_type, sign_types sign_type> struct uniform_digit_generator {
	...
	struct vn_op {
		template<typename v_type> VN_INLINE static char* convert(v_type v, char* buf, char* end) noexcept {
