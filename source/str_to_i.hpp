...
namespace benchmarks {
	...
	template<uint64_t total_size, typename int_type, sign_types sign_type> struct string_generator {
	...
	template<typename Op> struct from_chars_benchmark {
		template<typename size_type_01, typename vector_type, typename output_data_type, typename index_type> VN_FORCE_INLINE static uint64_t impl(
			size_type_01 test_size, const vector_type& test_data, output_data_type& output_buffer, index_type& current_index) {
	...
	template<uint64_t total_size, typename int_type, sign_types sign_type> struct leading_zero_string_generator {
